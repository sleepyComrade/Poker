...
export class GameLogic {
  ...
  private setNextPlayer() {
    ...
    if (this.players.every(player => player.isFold || player.chips === 0 || player.isAllIn)) {
      ...
      this.setNextPlayer();
    } else this.onMessage({type: 'ask', data: {actions: this.getActions(), playerId: this.currentPlayerIndex, raiseRange: this.getRaiseRange(), blind: this.minimalBet, pot: this.pot, askId: Date.now()}});
  }
